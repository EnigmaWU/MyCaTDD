...
4. **Stop** — Exit when the stage's gate passes. Update the TC status marker without deleting design comments, and report the verification result.
   - Bounded regeneration budget ($B \le 3$): If attempts to turn the test `GREEN` fail after 3 correction cycles, halt the loop, revert local unverified edits to prevent dirty workspace contamination, mark the TC as `🚫 BLOCKED`, and output a structured diagnostic report for developer intervention.
