...
1. Identify the span endpoint: the active story in `.catdd/spec/doingUS/` for `pre_close`, or the closed story in `.catdd/spec/doneUS/` for `post_close`. Require the paired `*-UserStory-Tasks.md` artifact.
2. Resolve the span scope: story and tasks artifacts, product code and test files, project-root `README*` SPEC docs, and, for `post_close`, the close-generated lifecycle/meta changes such as the `doingUS -> doneUS` move, story-link normalization, `README_UserStories.md` TODO/DONE and AC sync, and project-context updates.
...
- `commit_checkpoint`: `pre_close` or `post_close`.
...
- Story-span commit covering `SPEC_openUserStory -> SPEC_closeUserStory`, including close-generated lifecycle/meta changes when the checkpoint is `post_close`.
