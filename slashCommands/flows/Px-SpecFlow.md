...
After `SPEC_commitWorks`, if the work was done on a dedicated story branch, run the repository merge step (for example `SPEC_mergeWork` when available) before `SPEC_closeUserStory`.
...
23. Use [SPEC_commitWorks](../commands/Px-SpecFlow/SPEC_commitWorks.md), run merge/integration when required (for example `SPEC_mergeWork`), and then use [SPEC_closeUserStory](../commands/Px-SpecFlow/SPEC_closeUserStory.md) to finish the lifecycle; enforce the close-commit checkpoint when close-generated lifecycle/meta files were changed.
