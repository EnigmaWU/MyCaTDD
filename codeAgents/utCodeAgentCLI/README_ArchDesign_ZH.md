...
`utCodeAgentCLI` 采用分阶段运行时决策：**V1（PoC）使用 TypeScript on Node.js**，**V2（生产分发）预选 Go**。**Python** 经评估后未被选用。

决策理由摘要：

- 运行时语言需要和当前仓库的文档与 adapter 模型相匹配。
- 运行时语言需要尽量减少本地文件编排、命令路由与 machine-readable trace 生成的摩擦。
- 运行时语言需要保持清晰的 adapter 边界，以便未来加入 Copilot、OpenCode 或其他 runtime surface，而不改动 CaTDD 语义。
- 运行时语言需要让最终的 V1 实现对团队来说现实可建、可测、可维护。

完整决策记录见 [ADRs/ADR_RuntimeLanguage.md](ADRs/ADR_RuntimeLanguage.md)。
...
第二个风险是 runtime lock-in。CLI 可能需要根据最终选择支持 raw TypeScript/Node.js、Python 或 Go，再适配 Copilot-native 与 OpenCode surfaces，同时将 LangGraph 与 Google ADK 保持为 research-informed optional adapters。
...
- 支持一个可承载 raw TypeScript/Node.js、Python 或 Go 的 runtime shape，具体取决于评审后的取舍。
...
V1 实现目标是 Node.js TypeScript CLI：读取本地文件，通过 internal runner 或 process adapter 执行 portable slash-command steps，写入确定性 traces，并且不要求外部 agent runtime。V1 选择该 runtime 的原因是目标 adapter 生态（Copilot SDK、MCP、OpenCode）本身就是 Node/TypeScript-native，可获得一流 adapter 而无需跨语言桥接。Go 预选用于 V2 以获得 single-binary 生产分发；AgentSDK/adapter 边界保持 runtime-portable，使 V2 迁移成本受控。
...
本次修订用于响应当前活动 story 的后续架构反馈 [../../.catdd/spec/doingUS/20260606-harden-utCodeAgentCLI-agentic-reliability-UserStory.md](../../.catdd/spec/doingUS/20260606-harden-utCodeAgentCLI-agentic-reliability-UserStory.md)，并保持该 story 为 design-oriented-only。
...
| --- | --- |
| Retry and iteration budget | 参见 ASR-R1 与 ADR v1 默认值。`SlashCommandExecutor` 负责有界重试/修正与预算耗尽后的确定性路由。 |
| Unknown `--behave` fallback | 参见 ASR-R2 与 ADR fallback 决策。`BehaviorRegistry` 禁止静默容错，未知行为进入 diagnostics fallback 并以参数错误退出。 |
| Failure taxonomy and routing | 参见 ASR-R3 与 ADR taxonomy 决策。失败分类明确，且按类别执行不同路由（可重试或立即失败）。 |
| Snapshot and rollback boundary | 参见 ASR-R4 与 ADR boundary 决策。失败处理采用 step 级一致性边界与补偿式停止策略。 |
| Escalation threshold and non-interactive behavior | 参见 ASR-R5 与 ADR escalation 决策。升级触发条件明确，非交互行为具备确定性。 |
| Shell safety and sensitive-file protection | 参见 ASR-R6 与 ADR safety 决策。执行采用 allowlist-first，敏感路径默认拒绝，并约束 trace redaction。 |

### Architecture Feedback Checklist

| Feedback item | Status | Notes |
| --- | --- | --- |
| Add explicit retry and iteration limits | Addressed | 已补充数值预算与耗尽时的确定性行为。 |
| Add fallback route for unknown `--behave` | Addressed | 已补充 diagnostics-only fallback 与明确参数错误退出。 |
| Add transient vs permanent failure routing | Addressed | 已补充两类失败分类与路由策略。 |
| Add rollback or compensation boundary | Addressed | 已补充 step 级快照与补偿边界。 |
...
| --- | --- | --- |
| 在 `utCodeAgentCLI` 下方引入通用 `AgentSDK`。 | 保持 LLM runtime concerns 可复用且不依赖 CaTDD。 | Accepted. |
| V1 采用 TypeScript/Node.js，V2 预选 Go。 | V1 侧重 adapter-native 的 PoC 速度；V2 侧重 single-binary 生产分发。 | Accepted. |
| 将 Copilot/MCP 与 OpenCode 作为 adapter targets。 | 满足 compatibility goals，同时保持 CLI core 稳定。 | Accepted. |
...
运行时语言决定已记录为 ADR，采用分阶段结论：V1 使用 TS/Node，V2 预选 Go。

| Candidate | Evaluation summary | Outcome |
| --- | --- | --- |
| TypeScript / Node.js | adapter-native（Copilot/MCP/OpenCode），与本地文件编排、trace 生成与 adapter-first CLI 设计契合度高；对 PoC 而言集成成本最低。 | Selected for V1 |
| Python | scripting 生态强、编排迭代速度快，但 adapter 集成需要跨语言桥接，对该 CLI 而言其成本超过 scripting 速度收益。 | Not selected |
| Go | binary/runtime 简洁、部署紧凑；最适合生产 single-binary 分发。 | Pre-selected for V2 |

## Quality Attribute Scenarios (Measured)

| ID | Source | Stimulus | Environment | Response | Response Measure |
| --- | --- | --- | --- | --- | --- |
| QAS-1 Modifiability | Maintainer | 增加一个新的 runtime adapter target（例如新的 host/runtime surface）。 | 使用现有 `AgentSDK` contracts 的正常开发环境。 | 新增 adapter 时不改动 CaTDD 语义，不重写跨模块核心 contract。 | `AgentSDK` 核心改动 <= 2 个文件；不需要改动 `methodPrompts/` 或 `slashCommands/` 语义。 |
| QAS-2 Diagnosability | Developer | 委托 slash-command step 在 CI 或本地执行失败。 | command execution 期间的 runtime failure。 | 系统输出 structured trace 与可执行诊断信息，包含 failure point 与 completed steps。 | 从检测到失败起 <= 10 秒内，trace 包含 failed step ID、command path 与 error summary。 |
| QAS-3 Runtime Resilience | Operator/Developer | 多步骤执行时收到 abort/timeout/cancel。 | 本地或托管 adapter 的受控中断场景。 | 执行安全停止，保留已完成步骤证据，避免非法 TC 状态变更。 | 100% 中断运行都持久化 execution-failure trace，且不记录非法 TC transition。 |

## Sensitivity And Tradeoff Points

### Sensitivity Points

- `AgentSDK` adapter boundary 的严格程度对 modifiability 与迁移成本高度敏感。
- Trace schema 完整性对 diagnosability 与故障后恢复速度高度敏感。

### Tradeoff Points

- 更丰富的 diagnostics 与 trace 细节提升可诊断性，但会增加 runtime overhead 与 redaction 复杂度。
- 更强的 adapter 解耦提升可移植性与长期演进能力，但会增加前期接口设计复杂度。

## Inter-View Consistency Log

- Context vs Container：Context view 中的外部 runtime surfaces（Raw TS、Copilot/MCP、OpenCode、Existing CLI）均由 adapter/runtime containers 承接。
- Functional/Component vs Development：组件职责（`CliParser`、`CatddPlanner`、`SlashCommandExecutor`、`TraceWriter`）与 `src/cli`、`src/catdd`、`src/executor`、`src/trace` 模块边界一致。
- Concurrency/Execution vs Deployment：runtime execution 与 control/approval 流程可映射到 deployment targets 与 adapter boundaries，且不改变 CaTDD 语义归属。
...
- Architecture decisions 可追溯到 architecture-changing baseline/update stories 与 role-specific requirements。
