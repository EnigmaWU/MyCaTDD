...
`utCodeAgentCLI` 的运行时语言正在评估中。当前主要候选是 **TypeScript on Node.js**、**Python** 与 **Go**。
...
一个候选实现目标是 Node.js TypeScript CLI：读取本地文件，通过 internal runner 或 process adapter 执行 portable slash-command steps，写入确定性 traces，并且不要求外部 agent runtime。其他候选（如 Python 与 Go）应在固定 V1 选择前按同样标准比较。
...
运行时语言决定已记录为 ADR，并且当前用于比较实现选项，而不是预设某一个答案。

| Candidate | Evaluation summary | Outcome |
| --- | --- | --- |
| TypeScript / Node.js | 与当前仓库 TypeScript-facing 文档、本地文件编排、trace 生成与 adapter-first CLI 设计契合度高，但仍需结合实现成本与团队工作流适配度做最终取舍。 | Candidate |
| Python | scripting 生态强、编排迭代速度快，但会拉大与当前 TypeScript-facing 设计文档之间的翻译成本。 | Candidate |
| Go | binary/runtime 简洁、部署紧凑，但对一个以编排为主的 CLI 来说通常意味着更大的架构与工具链迁移成本。 | Candidate |
