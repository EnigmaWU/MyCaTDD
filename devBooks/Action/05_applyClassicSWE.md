...
**`user-story-centered-spec-coding`** — Packages the SpecCoding lifecycle as a reusable workflow. It tells the agent how to move work through pendingNews → todoUS → doingUS → doneUS, with abortUS preserving unsafe active stories for later analysis, CaTDD as the default testing method, and typical TDD as an alternative when requested. The skill bundles the complete Px-SpecFlow lifecycle: flow document, all 27 `SPEC_*` command files, project-root README SPEC templates, and CaTDD method references as the default testing engine.
