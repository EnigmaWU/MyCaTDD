...
Options:
  ...
  --clean               Remove existing generated UT_*.prompt and SPEC_*.prompt files from the output directory first.
...
if [[ "$CLEAN" -eq 1 ]]; then
  find "$OUTPUT_DIR" -maxdepth 1 -type f \( -name 'UT_*.prompt' -o -name 'SPEC_*.prompt' \) -delete
fi
