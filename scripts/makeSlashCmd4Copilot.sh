...
OUTPUT_DIR="$REPO_ROOT/.github/prompts"
WORKSPACE_ROOT="$REPO_ROOT"
CLEAN=0

usage() {
  cat <<'USAGE'
Usage: scripts/makeSlashCmd4Copilot.sh [--source-dir DIR] [--output DIR] [--workspace-root DIR] [--clean]

Generate Copilot-native .prompt.md wrappers from portable slashCommands.
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    --source-dir)
      [[ $# -ge 2 ]] || { echo "[makeSlashCmd4Copilot] --source-dir requires a directory" >&2; exit 2; }
      SOURCE_DIR="$2"
      shift 2
      ;;
    --output)
      [[ $# -ge 2 ]] || { echo "[makeSlashCmd4Copilot] --output requires a directory" >&2; exit 2; }
      OUTPUT_DIR="$2"
      shift 2
      ;;
    --workspace-root)
      [[ $# -ge 2 ]] || { echo "[makeSlashCmd4Copilot] --workspace-root requires a directory" >&2; exit 2; }
      WORKSPACE_ROOT="$2"
      shift 2
      ;;
    --clean)
    ...
    -h|--help)
      ...
      ;;
    *)
      echo "[makeSlashCmd4Copilot] Unknown argument: $1" >&2
      usage >&2
      exit 2
      ;;
  esac
...
if [[ ! -d "$SOURCE_DIR" ]]; then
  echo "[makeSlashCmd4Copilot] Missing source directory: slashCommands/commands" >&2
  exit 1
fi

if [[ ! -d "$WORKSPACE_ROOT" ]]; then
  echo "[makeSlashCmd4Copilot] Missing workspace root: $WORKSPACE_ROOT" >&2
  exit 1
fi
...
if [[ ! -d "$METHOD_ROOT" ]]; then
  echo "[makeSlashCmd4Copilot] Missing methodPrompts sibling for source: $METHOD_ROOT" >&2
  exit 1
fi
