...
log_install_operation() {
  ...
  echo "[installCaTDD4Antigravity] ${action}: ${path#$TARGET_DIR/}"
}
...
mkdir -p "$CATDD_DIR" "$SPEC_DIR/pendingNews" "$SPEC_DIR/analyzedNews" "$SPEC_DIR/todoUS" "$SPEC_DIR/doingUS" "$SPEC_DIR/suspendUS" "$SPEC_DIR/abortUS" "$SPEC_DIR/doneUS" "$ANTIGRAVITY_RULES_DIR"
