...
grep -Fq 'same evidence' "$instructions" || fail "instructions missing same-evidence evolution hook suppression"
...
harness_diagnose_sample="$TARGET_DIR/.github/prompts/HARNESS_diagnoseInstallation.prompt.md"
[[ -f "$harness_diagnose_sample" ]] || fail "missing installed HARNESS diagnose prompt"
grep -Fq '.catdd/slashCommands/commands/Px-HarnessKits/HARNESS_diagnoseInstallation.md' "$harness_diagnose_sample" || fail "HARNESS diagnose prompt does not point to installed slashCommands"
grep -Fq '.catdd/methodPrompts/README.md' "$harness_diagnose_sample" || fail "HARNESS diagnose prompt does not point to installed methodPrompts"
...
[[ -f "$init_target/.github/prompts/HARNESS_diagnoseInstallation.prompt.md" ]] || fail "--init target missing generated HARNESS diagnose Copilot prompt"
