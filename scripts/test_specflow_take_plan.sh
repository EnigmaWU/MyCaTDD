...
grep -Fq '.catdd/spec/doingUS/*-PLANING.md' "$TAKE_PLAN" || fail "SPEC_takePlan must create a paired PLANING artifact in doingUS"
...
grep -Fq '.catdd/spec/doneUS/*-PLANING.md' "$CLOSE_STORY" || fail "SPEC_closeUserStory must preserve the paired PLANING artifact in doneUS"

grep -Fq '[SPEC_takePlan.md](SPEC_takePlan.md)' "$COMMAND_README" || fail "Px-SpecFlow command README must list SPEC_takePlan"
grep -Fq 'Plan the next SPEC step for an opened user story' "$USER_GUIDE" || fail "slashCommands user guide must map the planning command"

grep -Fq 'SPEC_takePlan' "$FLOW_DOC" || fail "Px-SpecFlow must include SPEC_takePlan"
grep -Fq '.catdd/spec/doingUS/*-PLANING.md' "$FLOW_DOC" || fail "Px-SpecFlow must document the active PLANING artifact"
grep -Fq '.catdd/spec/doneUS/*-PLANING.md' "$FLOW_DOC" || fail "Px-SpecFlow must document the completed PLANING artifact"
