...
grep -Fq 'Distinguish design-oriented work from implementation-oriented work' "$TAKE_PLAN" || fail "SPEC_makePlan must distinguish design-oriented and implementation-oriented work"
...
grep -Fq '### Part 2.a: Design-Oriented Active Story Lifecycle' "$FLOW_DOC" || fail "Px-SpecFlow must split Part 2.a as design-oriented lifecycle"
...
grep -Fq 'ImplementationChoice -- "design ready, needs story review" --> ReviewStoryRef["return to Part 2.a SPEC_reviewUserStory"]' "$FLOW_DOC" || fail "Px-SpecFlow must route design-ready work back to story review"
...
grep -Fq 'QualityStory -- "YES" --> TailChoice{"after design, what story type?"}' "$FLOW_DOC" || fail "Px-SpecFlow must decide at end of Part 2.a whether to implement or close"
