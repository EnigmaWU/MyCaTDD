...
usage() {
  cat <<'USAGE'
Usage: scripts/installCaTDD4Cline.sh --target DIR [--init] [--verbose]
...
mkdir -p "$CATDD_DIR" "$SPEC_DIR/pendingNews" "$SPEC_DIR/analyzedNews" "$SPEC_DIR/todoUS" "$SPEC_DIR/doingUS" "$SPEC_DIR/doneUS" "$CLINE_RULES_DIR"
