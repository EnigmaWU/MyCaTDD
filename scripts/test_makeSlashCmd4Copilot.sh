...
git -C "$REPO_ROOT" check-ignore -q .github/prompts/HARNESS_example.prompt.md || fail "generated HARNESS prompt wrappers must be ignored in this source repo"
...
prompt_count="$(find "$OUT_DIR" -type f \( -name 'UT_*.prompt.md' -o -name 'SPEC_*.prompt.md' -o -name 'HARNESS_*.prompt.md' \) | wc -l | tr -d '[:space:]')"
...
harness_sample="$OUT_DIR/HARNESS_patchCaTDDSource.prompt.md"
[[ -f "$harness_sample" ]] || fail "missing sample prompt: HARNESS_patchCaTDDSource.prompt.md"
grep -Fq 'description: "Run CaTDD slash command HARNESS_patchCaTDDSource"' "$harness_sample" || fail "HARNESS sample prompt missing Copilot description"
grep -Fq 'slashCommands/commands/Px-HarnessKits/HARNESS_patchCaTDDSource.md' "$harness_sample" || fail "HARNESS sample prompt missing source command reference"
grep -Fq 'methodPrompts' "$harness_sample" || fail "HARNESS sample prompt missing methodPrompts source-of-truth reference"

harness_verify_sample="$OUT_DIR/HARNESS_verifyInstallation.prompt.md"
[[ -f "$harness_verify_sample" ]] || fail "missing sample prompt: HARNESS_verifyInstallation.prompt.md"
grep -Fq 'description: "Run CaTDD slash command HARNESS_verifyInstallation"' "$harness_verify_sample" || fail "HARNESS verify sample prompt missing Copilot description"
