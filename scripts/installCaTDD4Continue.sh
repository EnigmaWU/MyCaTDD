...
Options:
  ...
  --clean-prompts   Remove existing generated UT_*.prompt, SPEC_*.prompt, and HARNESS_*.prompt files before regenerating Continue wrappers.
...
mkdir -p "$CATDD_DIR" "$SPEC_DIR/pendingNews" "$SPEC_DIR/analyzedNews" "$SPEC_DIR/todoUS" "$SPEC_DIR/doingUS" "$SPEC_DIR/suspendUS" "$SPEC_DIR/abortUS" "$SPEC_DIR/doneUS" "$CONTINUE_RULES_DIR" "$CONTINUE_PROMPTS_DIR"
