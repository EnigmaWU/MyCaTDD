...
mkdir -p "$CATDD_DIR" "$SPEC_DIR/pendingNews" "$SPEC_DIR/analyzedNews" "$SPEC_DIR/todoUS" "$SPEC_DIR/doingUS" "$SPEC_DIR/suspendUS" "$SPEC_DIR/abortUS" "$SPEC_DIR/doneUS" "$CUSTOM_RULES_DIR" "$CUSTOM_PROMPTS_DIR"
...
if [[ ! -f "$ROOT_UBILANG_SOURCE" ]]; then
  echo "[installCaTDD4Custom] Missing source file: $ROOT_UBILANG_SOURCE" >&2
  exit 1
fi
if [[ ! -f "$ROOT_UBILANG_ZH_SOURCE" ]]; then
  echo "[installCaTDD4Custom] Missing source file: $ROOT_UBILANG_ZH_SOURCE" >&2
  exit 1
fi
