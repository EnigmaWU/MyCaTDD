...
面向将 CaTDD 打包为可复用 agent skills 的开发者与 CodeAgent 的实践指南。
...
`agentSkill/` 将 CaTDD 技能源码转化为自包含生成包。
...
- 想将 CaTDD 作为可复用 CodeAgent capability 分发。
- 发布或复制前需要确认生成包是自包含的。
...
技能包让 CodeAgent 获得紧凑、可复用的 CaTDD capability，同时避免技能源码树暴露重复链接路径或不完整参考资料。

这样既保持日常仓库编辑清爽，又能生成包含方法指南、方法提示词、实现模板和斜杠命令流程的可分发产物。

## 方法

打包 CaTDD skill 时，按以下流程执行。

1. 编辑 `agentSkill/comment-alive-test-driven-development/` 下的技能源码。
2. 将方法资产保留在 `methodPrompts/`，将命令资产保留在 `slashCommands/`。
3. 在仓库根目录运行 `bash agentSkill/makeSkill.sh`。
...
在仓库根目录运行以下命令，将技能包生成到临时输出目录并验证关键文件：
...
- `test` 命令成功退出。
- 符号链接数量输出 `0`。
- 输出的临时路径包含自包含的 `comment-alive-test-driven-development/` 包。

## 打包输出

`bash agentSkill/makeSkill.sh` 默认生成的包包含：
