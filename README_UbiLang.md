...
| SPEC_commitPreStoryWorks | Story-span command that commits the pre-story interval: import/analysis artifacts, `pendingNews/` to `analyzedNews/` moves, `todoUS/` stories, and the `README_UserStories.md` ledger produced before `SPEC_openUserStory`. Available as an option in `manualMode`; the default pre-story checkpoint in `autonomousMode` when the intake ran headless with `analysis_mode: AUTONOMOUS`. |
...
| SPEC_commitStoryWorks | Story-span command that commits the whole `SPEC_openUserStory -> SPEC_closeUserStory` interval as the just-done UserStory commit, including close-generated lifecycle/meta changes. It serves the `pre_close` checkpoint (providing `commit_ref` for `SPEC_closeUserStory`) and the `post_close` checkpoint (satisfying `close_commit_required`). Available as an option in `manualMode`; the default at story completion in `autonomousMode`. |
